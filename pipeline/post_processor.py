...
def remove_duplicates(questions: list[dict], report: ValidationReport) -> list[dict]:
    ...
    for q in questions:
        ...
        if "c" in q:
            fingerprint_parts.append(q["c"].strip())
...
def check_code_accuracy(questions: list[dict], report: ValidationReport):
    """
    Spot-check 'What is the output?' questions by running the code.
    Only checks simple, safe code snippets.
    """
    unsafe_modules = {"os", "sys", "subprocess", "shutil", "signal", "ctypes",
                      "socket", "http", "urllib", "pathlib", "tempfile",
                      "gc", "threading", "multiprocessing",
                      "transformers", "torch", "tensorflow", "huggingface_hub", "datasets", "langchain"}

    for i, q in enumerate(questions):
        if q.get("type") != "mcq" or "c" not in q:
            continue

        qtext = q.get("q", "").lower()
        if "output" not in qtext:
            continue

        code = q["c"]

        # Skip unsafe code
        if any(f"import {mod}" in code or f"from {mod}" in code for mod in unsafe_modules):
            continue

        # Skip non-deterministic code
        if "random" in code or "time.time" in code or "id(" in code:
            continue

        # Skip complex code (multi-class, long)
        if len(code) > 200 or "class " in code:
            continue
        ...
        try:
            ...
            actual_answer = str(q["o"][0]).strip() if q.get("o") else ""

            if expected and actual_answer and expected != actual_answer:
                report.accuracy_warnings.append(
                    f"Q#{i}: Code output mismatch — expected '{expected}', answer is '{actual_answer}'"
                )
        except Exception:
            pass  # Complex or intentionally erroring code
...
def process_questions(
    ...
    # Step 4: Check code accuracy
    check_code_accuracy(valid_questions, report)

    # Step 5: Compute type distribution
