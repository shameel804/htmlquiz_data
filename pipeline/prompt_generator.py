...
9. **Verify all code snippet outputs mentally before writing**. If a question asks "What is the output of this code?", the first option MUST be the actual output of that code.
10. **Avoid version-dependent answers**: Don't ask about behavior that differs between versions (e.g., HTML4 vs HTML5).
11. **Avoid non-deterministic code**: Don't use `random`, `time`, or `id()` in output questions.
...
17. **NO GENERATION SCRIPTS**: You MUST use your AI knowledge to manually generate every single question. DO NOT write a script (or any other code) to programmatically generate the datasets. Script-generated questions are low quality and strictly forbidden. Pacing across turns is permitted to avoid token limits.
"""

CODE_SNIPPET_RULES = """
## Code Snippet Rules
* If code is included, place it inside the `"c"` field as a JSON string.
* Use `\\n` for new lines.
* Code must NOT reveal the correct answer.
* Code should use spaces instead of tabs.
* Code should be syntactically correct HTML/CSS/JS that actually works.
"""
...
[
    {{
        "q": "Which of the following is a valid semantic tag in HTML?",
        "type": "mcq",
        "o": [
            "var_1",
            "$var",
...
- `"q"`: The question text.
- `"c"`: (Optional) Code snippet as a JSON string with `\\n` for newlines.
...
- **80%+ questions should be WITHOUT code snippets** (conceptual/knowledge questions).
- **Below 20% questions should have code snippets** (in the `"c"` field).
...
[
    ...
    {{
        "q": "The ______ function is used to check the data type of a variable.",
        "type": "fill_blank",
        "answers": ["type"],
        "other_options": ["data", "check", "info"]
    }},
    {{
        "q": "Match the stages with their descriptions:",
        "type": "match",
        "left": ["Data Collection", "Data Preparation", "Model Building", "Deployment"],
        "right": ["Gathering raw data", "Cleaning data", "Training models", "Putting into production"]
    }},
    {{
        "q": "Feature engineering occurs during the model building phase.",
        "type": "true_false",
        "correct": "False"
    }},
    {{
        "q": "In what year was JavaScript first released?",
        "type": "slider",
        "min": 1980,
        "max": 2010,
        "correct": 1991
    ...
    {{
        "q": "Match the term with its definition by flipping pairs.",
        "type": "memory",
        "pairs": {{
    ...
    {{
        "q": "Which of these is not a valid CSS property?",
        "type": "odd_one_out",
        "options": ["int", "float", "double", "str"],
        "correct": "double"
    }},
    {{
        "q": "Swipe RIGHT for mutable types, LEFT for immutable types",
        "type": "swipe_cards",
        "left_label": "Immutable",
        "right_label": "Mutable",
        "cards": {{
            "list": "right",
            "tuple": "left",
    ...
    {{
        "q": "What is the keyword used to define a function?",
        "type": "word_scramble",
        "answer": "DEF",
        "decoys": "XQZ"
    }},
    {{
        ...
        "text": "The built-in function [0] outputs data, and [1] takes user input.",
        "dropdowns": [
            {{"correct": "print()", "options": ["read()", "print()", "show()"]}},
            {{"correct": "input()", "options": ["get()", "input()", "read()"]}}
        ]
    }}
]
```
...
- Keys = terms/code, Values = definitions/descriptions.
...
## Overview
You are generating quiz data for a production HTML/CSS/JS quiz app. This file contains
...
- **ABSOLUTELY NO GENERATION SCRIPTS**: Do NOT attempt to create scripts, programs, or templates to generate the quiz content. Script-generated questions lack depth and quality, and are strictly forbidden. You must write the questions directly using your own LLM intelligence.
...
- NO GENERATION SCRIPTS: You MUST generate the questions using your LLM knowledge. DO NOT write a script (or any other code) to programmatically generate the questions.
