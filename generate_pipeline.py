...
Usage:
    ...
    python3 generate_pipeline.py prompts "Html"
    python3 generate_pipeline.py prompts --all

    # Generate prompts for a single topic
    python3 generate_pipeline.py prompts "Html" --topic basic

    # Process staged AI outputs → validate + merge + output
    python3 generate_pipeline.py process "Html"
    python3 generate_pipeline.py process "Html" --topic basic
    python3 generate_pipeline.py process --all

    # Validate an existing JSON file
    python3 generate_pipeline.py validate v4/Html/basic.json

    # Show status of all topics
    python3 generate_pipeline.py status
    python3 generate_pipeline.py status "Html"
"""
import argparse
import json
import sys
...
Examples:
  ...
  python3 generate_pipeline.py status
    ...
    prompts_parser.add_argument("category", nargs="?", help="Category name (e.g., 'Html')")
